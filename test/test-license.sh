...
run_ok '$status -eq 0' license -l
run_ok '$status -eq 0' license -d test-license -u
run_ok '$status -eq 0 && $(echo $output | wc -l) -gt 0' license -d test-license -l
run_ok '$status -eq 0 && $output =~ "XXXXXX"' license -d test-license mit
run_ok '$status -eq 0 && $output =~ "XXXXXX"' license -d test-license unlicense
